...
import it.csi.sicee.siceesrv.business.dao.dto.SiceeTDetImp2015Pk;
import it.csi.sicee.siceesrv.business.dao.exceptions.SiceeTDetImp2015DaoException;
...
public class SiceeTDetImp2015DaoImpl extends AbstractDAO implements ParameterizedRowMapper<SiceeTDetImp2015>, SiceeTDetImp2015Dao
{
	...
	/** 
	 * Returns all rows from the SICEE_T_DET_IMP_2015 table that match the criteria 'ID_CERTIFICATORE = :idCertificatore AND PROGR_CERTIFICATO = :progrCertificato AND ANNO = :anno AND FK_SERV_ENER = :fkServEner'.
	 */
	
	public List<SiceeTDetImp2015> findByServizioEnergetico(String idCertificatore, String progrCertificato, String anno, Integer fkServEner) throws SiceeTDetImp2015DaoException
	{
		try {
			return jdbcTemplate.query("SELECT ID_DETTAGLIO_IMP, PROGRESSIVO_DETT, ID_CERTIFICATORE, PROGR_CERTIFICATO, FK_COMBUSTIBILE, ANNO, FK_SERV_ENER, TIPO_IMPIANTO, ANNO_INSTALL, POTENZA_NOMIN_KW, CODICE_IMPIANTO_CIT, FK_TIPO_IMPIANTO FROM " + getTableName() + " WHERE ID_CERTIFICATORE = ? AND PROGR_CERTIFICATO = ? AND ANNO = ? AND FK_SERV_ENER = ? ORDER BY PROGRESSIVO_DETT", this, idCertificatore, progrCertificato, anno, fkServEner);
		}
		catch (Exception e) {
			throw new SiceeTDetImp2015DaoException("Query failed", e);
		}
