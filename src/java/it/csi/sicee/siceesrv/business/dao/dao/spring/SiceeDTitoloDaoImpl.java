...
public class SiceeDTitoloDaoImpl extends AbstractDAO implements ParameterizedRowMapper<SiceeDTitolo>, SiceeDTitoloDao
{
	...
	{
		jdbcTemplate.update("INSERT INTO " + getTableName() + " ( ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA ) VALUES ( ?, ?, ?, ?, ? )",dto.getIdTitolo(),dto.getDescr(),dto.getFlgEdifici(),dto.getFlgImpianti(),dto.getSigla());
		return dto.createPk();
	}
	...
	{
		jdbcTemplate.update("UPDATE " + getTableName() + " SET ID_TITOLO = ?, DESCR = ?, FLG_EDIFICI = ?, FLG_IMPIANTI = ?, SIGLA = ? WHERE ID_TITOLO = ?",dto.getIdTitolo(),dto.getDescr(),dto.getFlgEdifici(),dto.getFlgImpianti(),dto.getSigla(),pk.getIdTitolo());
	}
	...
	{
		try {
			List<SiceeDTitolo> list = jdbcTemplate.query("SELECT ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA FROM " + getTableName() + " WHERE ID_TITOLO = ?", this,idTitolo);
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA FROM " + getTableName() + " ORDER BY ID_TITOLO", this);
		}
		catch (Exception e) {
			throw new SiceeDTitoloDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA FROM " + getTableName() + " WHERE ID_TITOLO = ? ORDER BY ID_TITOLO", this,idTitolo);
		}
		catch (Exception e) {
			throw new SiceeDTitoloDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA FROM " + getTableName() + " WHERE DESCR = ? ORDER BY DESCR", this,descr);
		}
		catch (Exception e) {
			throw new SiceeDTitoloDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA FROM " + getTableName() + " WHERE FLG_EDIFICI = ? ORDER BY FLG_EDIFICI", this,flgEdifici);
		}
		catch (Exception e) {
			throw new SiceeDTitoloDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA FROM " + getTableName() + " WHERE FLG_IMPIANTI = ? ORDER BY FLG_IMPIANTI", this,flgImpianti);
		}
		catch (Exception e) {
			throw new SiceeDTitoloDaoException("Query failed", e);
		}
	...
	{
		try {
			return jdbcTemplate.query("SELECT ID_TITOLO, DESCR, FLG_EDIFICI, FLG_IMPIANTI, SIGLA FROM " + getTableName() + " WHERE SIGLA = ? ORDER BY SIGLA", this,sigla);
		}
		catch (Exception e) {
			throw new SiceeDTitoloDaoException("Query failed", e);
		}
